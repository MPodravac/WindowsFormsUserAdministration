...
namespace DatabaseService
{
    public class Crud
    {
        ...
        {
            String sSqlConnectionString = ConfigurationManager.ConnectionStrings["DbConnection"].ConnectionString;
            //postavlja konekciju na string
            using (DbConnection oConnection = new SqlConnection(sSqlConnectionString)) //postavlja konekciju na bazu
            //oslobađa memoriju
            using (DbCommand oCommand = oConnection.CreateCommand())
            {
        ...
        public void AddNewUser(User oUser)
        {
            String sSqlConnectionString = ConfigurationManager.ConnectionStrings["DbConnection"].ConnectionString;
            //postavlja konekciju na string
            using (DbConnection oConnection = new SqlConnection(sSqlConnectionString)) //postavlja konekciju na bazu
            ...
            using (DbCommand oCommand = oConnection.CreateCommand())
            {
                oCommand.CommandText = "INSERT INTO users (USERNAME, PASSWORD, NAME, SURNAME) VALUES ('" + oUser.sUserFirstName + "', '" + oUser.sUserLastName + "', '" + oUser.sUserName + "', '" + oUser.sUserPassword + "');";
                oConnection.Open();
                using (DbDataReader oReader = oCommand.ExecuteReader())
                {
                }
